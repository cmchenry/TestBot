    ...
    public class MessagesController : ApiController
    {
        ...
        public async Task<Message> Post([FromBody]Message message)
        {
            if (message.Type == "Message")
            {
                ...
                string retMessage = $"{++counter} You said '{message.Text}' which is {length} characters";
                if (length > 10)
                    retMessage += $", aren't you smart!";

                Message replyMessage = message.CreateReplyMessage(retMessage);
                replyMessage.CreateReplyMessage($"Nice!");
